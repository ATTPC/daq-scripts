...
print "Rearranging files for experiment $expname, $runname \n";
...
for my $n (0 .. 9) {
  ...
  if ( chdir("$fetchdir/mm$n/$subpath") ) {
    ...
    for my $file (@files) {
      ...
      next if not ($1 eq $runname);
      my $newFileName = "CoBo$n\_$runname\_$3.graw";  # output as "CoBoX_run_XXXX_(timestamp).graw"
      my $rundir = $runname;
