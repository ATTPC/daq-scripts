...
Usage:
    ...
    ./fetch_run.py <exp_name> <run_name>
...
def fetch(exp_name, run_name):
    playbook_path = os.path.expanduser('~/daq-scripts/fetch-data/fetch-file.yml')
    vars_str = 'exp_name={e} run_name={r}'.format(e=exp_name, r=run_name)
...
def rearrange_files(exp_name, run_name):
    fetch_dir = os.path.join('/data', exp_name, 'temp')  # where the files are
    clean_dir = os.path.join('/data', exp_name)
...
def main():
    if len(sys.argv) < 3:
        sys.exit('Provide experiment name and run name')
    exp_name = sys.argv[1]
    run_name = sys.argv[2]

    fetch(exp_name, run_name)
    rearrange_files(exp_name, run_name)
